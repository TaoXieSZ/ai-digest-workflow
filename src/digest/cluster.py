...
_PROMPT = """你是一个 AI 资讯主题聚类器。给定下面 N 条 AI 圈条目（标题 + 内容片段），
把它们归到 %(min_topics)d-%(max_topics)d 个主题。每个主题:
- name: ≤ 12 字的中文标签（如 "Claude 4.7 发布"、"AI 编程工具"）
- summary: ≤ 60 字的一句话主题概括
- 包含 1-%(max_per_topic)d 个最相关的条目

不属于任何明显主题的条目（噪声、个人感想、无关闲聊）请直接丢弃；
返回的 item_ids 总数可以少于输入。
